...
**Осталось по Э12.7 (полировка):** синхрон открытия доски/презентации у
всех (сейчас локальный `stageView` — каждый сам жмёт «Доска»); эмодзи-
реакции в чате; демонстрация слайдов гостю; `handleRoomFinishedWebhook`/
`endLessonNow` — мёртвый код, снести в Э12.9. Проверка фикса копий доски
на 2 клиентах — за пользователем (логика: sync-wait + фикс. id).
