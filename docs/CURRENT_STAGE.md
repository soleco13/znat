...
> 2026-09-23: **Э14.2 закрыт** — приглашения в пространство (`school_invites`,
> модуль `invites`, страница `/admin/invites`, приём по ссылке
> `/s/:slug/invite/:code`). Только по прямой ссылке от админа, не открытый
> поиск. Атомарный "захват слота" инвайта в одной транзакции с созданием
> пользователя — сбой (дубликат email) откатывает и инкремент счётчика
> использований, инвайт не сгорает впустую. Проверено сквозным приёмом на
> живом сервере (создание/список/отзыв/предпросмотр через реальный HTTP,
> письмо поймано временным mailhog, второе использование single-use
> инвайта корректно отклонено 410). Следующие — Э14.3 (OAuth), Э14.4
> (полировка).
>
> 2026-09-23: **Э14.1 закрыт** — публичная self-signup регистрация
> (репетитор физлицо / ООО-пространство), подтверждение почты через
> собственный SMTP-релей, автологин по ссылке. Новый продуктовый рывок из
> «одна школа» в мультитенантный self-serve (§ раздел Э14 ниже, план
> зафиксирован в переписке с пользователем, отдельного файла-ТЗ пока нет).
> Остальные подэтапы — Э14.2 (инвайты в пространство), Э14.3 (OAuth
> Яндекс/ВК, блокирован на получении пользователем client_id/secret),
> Э14.4 (полировка `/s/:slug`) — следующими сессиями, не смешивать с Э12/Э13.
>
> 2026-09-07: **Э12 закрыт технически** — Э12.8 и Э12.9 доделаны (журнал
> ниже). Осталось только то, что требует живой среды: `docker compose up`
> с нуля и гейт этапа с LiveKit — за пользователем.
>
...
> По Э13 остаётся полировка (список в его разделе).
...
слайдов гостю. Проверка фикса копий доски на 2 клиентах — за
пользователем (логика: sync-wait + фикс. id).

**Э12.8 — назначение материалов уроку в админке (2026-09-07). Коммит
`ce3324c`.** «⋮» на строке урока → «Материалы урока»: диалог со списком
назначенных (снять — крестиком) и выбором из опубликованной библиотеки
(уже назначенные отфильтрованы). Бэкенд не трогали — `GET/POST/DELETE
/lessons/:id/materials` готовы с Э12.3, `POST` сразу отдаёт новый список.
Проверено вживую под admin на dev-БД: назначение, снятие, пустое состояние.

**Э12.9 — чистка, миграции, ревизия документов (2026-09-07). Три коммита.**

`0e4a331` — снос статус-машины урока (мёртвый код после Э12.4):
- миграция `0018_e12_9_drop_lesson_status.sql`: `lessons.status` /
  `started_at` / `ended_at`, enum `lesson_status`;
- `lessons`: `startLesson`/`endLesson`, `repo.updateLessonStatus`;
- `rooms`: `endLessonNow` + маршрут `POST /lessons/:id/end` (фронт его уже
  не звал); `handleRoomFinishedWebhook` **сохранён**, но урезан до реальной
  работы — LiveKit-сигнал «комната закрылась» освобождает таймер, Y.Doc и
  отметку активности, «завершать» урок больше нечем;
- `shared`: `lessonStatusSchema`/`LessonStatus`, поле `lessonStatus` в
  ответе `join`, WS-событие `lesson_status`, мёртвые `lessonResponseSchema`/
  `createLessonRequestSchema`; `web`: мёртвое состояние в `RoomPage`.
- **Важное место:** лимитер трафика Э6.5 (§10.8 ТЗ) висел на переходе
  `scheduled → live`. Новый триггер — «комната была пуста, входит персонал
  кроме методиста» (`presence.countConnected() === 0` до записи участника).
  Поведение то же: новая комната при перегрузе платформы открывается в
  Лекции; уже идущие уроки не трогаются.

`a63d161` — снос групп/классов, CSV-импорта учеников и роли `student`
(Э12.2/12.3 оставляли их «переходными заглушками»):
- миграции `0019_e12_9_drop_groups.sql` (таблицы `groups`/`group_members`,
  колонка `lessons.group_id`; писана руками — `DROP TABLE ... CASCADE` сам
  снимает FK, поэтому `DROP CONSTRAINT IF EXISTS`) и
  `0020_e12_9_drop_student_role.sql` (роль из enum; `DELETE FROM users
  WHERE role='student'` перед приведением типа — иначе `USING` падает);
- `users`: CRUD групп, `GET /users/me/groups`, `POST /users/import` +
  `csv.ts`, роуты модуля целиком под admin;
- `shared`: схемы групп/импорта, роль `student` из `roleSchema`;
- `web`: заглушка «библиотека недоступна ученику», роль-фоллбэк
  `"student"` в `AppShell` (роль стала nullable — до загрузки профиля
  пунктов меню нет, а не «как у ученика»).
- **Важное место:** в `recordings/service.ts` проверка «ученику нельзя»
  заменена не на удаление, а на **белый список персонала** — access-JWT со
  снесённой ролью живёт ещё до 15 минут после деплоя и не должен получить
  доступ к записям. Тесты этой ветки сохранены, фикстуры помечены как
  легаси-токен.

`43182ad` — ревизия `docs/ТЗ.md` и `docs/ПЛАН.md` под фактическую модель
(по ревизионной таблице §1 план-ТЗ): §1.1/1.2, §4.2 (создаёт урок только
admin; строка «Видеть ответы других» убрана), §7.3 + новый §7.4 (домашка =
список материалов), §8.1 (API целиком), §9 (схема), §10.11/10.15, §11 +
новый §11.1 «Гостевой доступ», деплой-гейт по активным участникам вместо
`lessons.status`. В `ПЛАН.md` добавлен раздел «Э12–Э13 — post-MVP
переработка» (в исходном роадмапе их не было). `CLAUDE.md` править не
понадобилось — там правила работы, а не продуктовая модель.

Гейты Э12.9: `pnpm -r build` без ошибок, тесты **367 + 54** зелёные,
`depcheck` чистый (317 модулей), все четыре миграции прогнаны на живой
dev-БД (Postgres 16 нативный + Memurai вместо Redis), `seed:dev` работает
с нуля, экраны проверены в браузере (Playwright MCP).

**Осталось по Э12 (за пользователем — нужна живая среда):**
`docker compose up` с нуля и гейт этапа (гость по ссылке → урок → доска →
задание → материал; admin создаёт урок и видит журнал посещений) — в этой
среде нет ни Docker, ни LiveKit, поэтому всё, что требует медиасервера,
проверить нельзя (см. [[project_video_platform_env_gaps]]).
...
**Э13 доп. 2 (2026-09-07) — конструкции по всем предметам + новый тип
вопроса `categorize` (запрос пользователя «внедряй всё», см.
`docs/Э13-конструкции-чеклист.md` — исследование H5P/QTI/Wordwall/Skysmart
перед реализацией). Три коммита по фазам, каждая проверена сборкой.**

- **Фаза A — конструкции.** `material-templates.ts`: `MaterialConstruct.
  subject` (раздел пикера) + `CONSTRUCT_SUBJECT_ORDER`; +65 конструкций по
  11 предметам (русский, литература, английский, математика, физика,
  химия, биология, география, история/обществознание, информатика,
  музыка/ИЗО/ОБЖ) — только из уже существующих блоков/типов, 0 нового
  кода на бэке. Новые хелперы конструктора: `image`/`audioBlock`/`table`/
  `matchingQuestion`/`orderingQuestion`/`clozeDropdownQuestion`.
  `construct-picker.tsx` переписан: группировка по предмету (порядок из
  `CONSTRUCT_SUBJECT_ORDER`) + поиск по названию/описанию/предмету/
  триггерам (~85 карточек плоским списком были бы нечитаемы). Слэш-меню
  (`slash-menu.tsx`) — группа `Конструкции · <предмет>` вместо одной
  общей.
- **Фаза B — блок «Спойлер».** shadcn `accordion`/`slider`/`carousel`
  (`pnpm dlx shadcn add` — CLI переформатировал `tailwind.config.js`
  целиком и снёс комментарий design-system, откачен `git checkout`,
  ничего нового там не требовалось; `carousel.tsx` CLI не дописал —
  застрял на интерактивном промпте про `button.tsx`, дописан вручную по
  исходнику shadcn v4 registry). Новый контент-блок `spoiler`
  (`packages/shared/materials.ts`: `spoilerBlockSchema`, НЕ ключ ответа —
  просто свёрнутый по умолчанию блок): Tiptap-нода `SpoilerBlock`
  (`editor/nodes.tsx`, по образцу `callout`, заголовок редактируется
  инлайн), `serialize.ts`, `block-factories.ts`, слэш-меню, рендер
  ученику через `Accordion` (`MaterialPlayer.tsx`). `carousel`/`slider`
  установлены, но пока не использованы — задел на галерею/флеш-карточки
  и `slider`-вопрос (раздел 2, волна 2 чеклиста).
- **Фаза C — тип вопроса `categorize` (тип 11, §6.3/§6.4 ТЗ — сверх
  стоп-листа Э8 «НЕ делать типы 11–22», прямой запрос пользователя;
  формула частичных баллов уже была в ТЗ, не самодеятельность).**
  Читалось построчно (§ «Что не делегировать вслепую» CLAUDE.md).
  Все точки касания: `packages/shared/materials.ts` (interaction/response
  схемы, `stripInteractionAnswerKey` — вырезает `categoryId`,
  `questionHasCorrectAnswer`), `apps/api/materials/grading.ts`
  (`partialCredit`, та же формула, что `matching`/`cloze_*` — табличные
  тесты в `grading.test.ts`, +4), `activities/analytics.ts`
  (`buildDistribution` → `summarize`, `formatResponseText`),
  `activities/service.ts` (`emptyResponseFor`), `answer-format.ts` (обе
  функции), `block-factories.ts` (`INTERACTION_LABELS` + `createInteraction`),
  `QuestionPlayer.tsx` + новый `CategorizePlayer`
  (`AdvancedInteractionPlayers.tsx`, тот же приём, что `MatchingPlayer`:
  DnD-корзины + `<select>` на элемент как клавиатурная альтернатива, §16
  ТЗ), `QuestionInteractionEditors.tsx` (`CategorizeEditor` +
  `CategoryListEditor`), слэш-меню. Каждое место найдено переборкой всех
  `case "ordering"` в репозитории + подтверждено чистым `tsc` после
  правки схемы (дискриминированное объединение само указывает
  пропущенные `case`). 4 конструкции-заглушки (части речи, металлы/
  неметаллы, позвоночные/беспозвоночные, природные зоны), раньше
  эмулировавшие категоризацию через `multiple_choice`, переведены на
  настоящий `categorize`.

Гейты после каждой фазы: `pnpm -r build`/`pnpm -r test` зелёные, `pnpm
depcheck` — без нарушений. В браузере руками не проверено (see
[[project_video_platform_env_gaps]]).

**Э13 доп. 3 (2026-09-07, тот же контекст) — «+»/«/» только через модалку
конструкций; типы 14 `highlight_text` и 16 `table_fill`. Запрос
пользователя: «доделывай» + «все шаблоны конструкций, которые
открываются при нажатии на "+", должны открываться в модальном окне, с
удобной фильтрацией и группировкой по предметам».**

- **Меню «+»/«/» — конструкции только через модалку.** `slash-menu.tsx`:
  плоский список ~75 конструкций в инлайн-меню убран — методисту мешал
  бы, а не помогал (доп. 2 как раз и делала эту модалку удобной). Один
  пункт «Конструкции — открыть каталог…», его `keywords` — метки/
  описания/предметы/триггеры ВСЕХ конструкций разом (поиск по «диктант»
  всё ещё находит пункт). `SlashOptions.onOpenConstructs` получил второй
  параметр `initialQuery` — то, что методист напечатал после «/» до
  выбора пункта, прокидывается в модалку и предзаполняет её поиск
  (`MaterialDocEditor.tsx` → `ConstructPickerDialog.initialQuery`, синхро
  через `useEffect` на `open`). «+» (`FloatingMenu`) как и раньше просто
  вставляет «/» — теперь ведёт в этот же короткий список.
- **`highlight_text`, тип 14 §6.3 ТЗ («выделить фрагменты в тексте»).**
  Модель — КАЖДОЕ слово текста отдельный токен `{id, text, correct}`
  (как варианты `multiple_choice`, только по словам, не по вариантам).
  Грейдинг — та же формула multiple_choice. Редактор
  (`HighlightTextEditor`): текстовое поле + «Разбить на слова» (best-effort
  сохраняет `correct` у слов с тем же текстом на той же позиции при
  повторном разбиении) + клик по слову переключает «искомое».
  Плеер (`HighlightTextPlayer`, `QuestionPlayer.tsx`) — чекбоксы
  `sr-only` + `peer-checked`, инлайн-текст, полностью с клавиатуры.
- **`table_fill`, тип 16 §6.3 ТЗ («заполнить ячейки таблицы»).** Ячейка —
  `{kind:"static", text}` или `{kind:"input", id, answers: TextMatchRule[],
  ...}` (та же модель сравнения, что гэп `cloze_text`). Грейдинг — формула
  `cloze_text` (частичный балл по input-ячейкам, незаполненная не
  штрафует). Редактор (`TableFillEditor`) — таблица с кнопкой на ячейку
  «→ поле ответа»/«→ обычный текст», +строка/+столбец/удалить. Плеер
  (`TableFillPlayer`) — `<table>` со статичными ячейками текстом и
  `Input` на входных.
- Все точки касания (grading/analytics/service/answer-format/player/
  editor/factories/slash-menu) снова найдены переборкой `case "ordering"`
  + подтверждены чистым `tsc` (дискрим. объединение). Таблица тестов в
  `grading.test.ts` (+7), тесты strip/validate в `materials.test.ts` (+6).
  6 конструкций-заглушек (синтаксический разбор, средства выразительности
  → `highlight_text`; лабораторная/сравнительная таблица/трассировка
  алгоритма → `table_fill`), раньше приближавшие эти типы через
  `text_input`/content-`table`, переведены на настоящие интерактивные типы.

Гейты: `pnpm -r build`/`pnpm -r test` (62 shared + 379 api, было 429 в
сумме на доп. 2 → 441 сейчас) зелёные, `pnpm depcheck` — без нарушений. В
браузере по-прежнему не проверено (среда).

**Э13 доп. 4 (2026-09-07) — материал-витрина всех конструкций.** Запрос
пользователя: «сделай материал, где я смогу просмотреть все конструкции…
заполни его нормально теорией и практикой, используй ВСЕ конструкции».

- `apps/api/src/db/build-construct-showcase.ts` (ручной инструмент, как
  `seed-material.ts` рядом — не рантайм). Строит один `Material`,
  использующий **все 75** конструкций `MATERIAL_CONSTRUCTS` с настоящим
  предметным содержанием (не заготовки редактора): реальные слова
  диктанта, формулы с реальными числами и верным ответом, реальные даты
  истории, реальный код Python, работающие уравнения химии (проверен
  баланс) и т.д. — не импортирует `material-templates.ts` из web
  (пересечение границы приложений, там ещё и lucide-react/React); id всех
  конструкций переписаны в `ALL_CONSTRUCT_IDS` вручную,
  `assertCoverage()` в конце бросает исключение, если конструкция
  осталась без контента.
- `mediaPlaceholder()` — для конструкций с `image`/`audio`: реальных
  файлов в этой среде взять неоткуда (`imageBlockSchema`/
  `audioBlockSchema` требуют непустой `assetId`, StorageAdapter/БД
  недоступны), поэтому такие блоки — `callout` с текстовым описанием
  того, что должно быть на картинке/в записи; методист заменит на
  реальный файл через медиатеку.
- Результат — `docs/materials/construct-showcase.json` (264 блока, 124
  вопроса, все 13 типов представлены). `materialSchema.parse` +
  `validateMaterialContent` — ноль проблем. Отдельно (временный скрипт,
  не оставлен в репозитории) прогнан через настоящий `gradeResponse`
  («идеальный» ответ на каждый автопроверяемый вопрос) — 114/114 дают
  полный балл, 10 `open_answer` корректно уходят на ручную проверку.
- `apps/api/package.json`: `pnpm --filter @school/api run build:showcase`
  пересобирает JSON (без БД). Завести материал в БД (нужен `docker
  compose up`, недоступен в этой среде — см.
  [[project_video_platform_env_gaps]]):
  ```
...
и старый роут); ТЗ §7.2 обновить под новый экран (и §6.3 — отметить ★ у
`categorize`/`highlight_text`/`table_fill`, раз они реализованы раньше
плана); удаление материала / чистка плейсхолдер-черновиков (нет UI);
горячая клавиша Ctrl+E; живая проверка в браузере всего из доп. 2/3
(среда не даёт `docker compose up`, см.
[[project_video_platform_env_gaps]]).

**Из `docs/Э13-конструкции-чеклист.md` реализовано:** раздел 3 целиком
(65+6 конструкций), раздел 4 частично (только «спойлер»), из волны 1
раздела 2 — `categorize` (тип 11), `highlight_text`/бывший «mark_text»
(тип 14), `table_fill` (тип 16). **Не реализовано** (задел на следующий
контекст): волна 1 раздела 2 — `image_labeling`/`drag_to_image` (тип 12),
`hotspot` (тип 13), `matrix_choice` (НЕ в ТЗ — своя добавка сверх 22
типов, к обсуждению), `drag_words` (UI-надстройка над `cloze_text`,
нового типа не требует), `canvas_answer` (в ТЗ нет прямого аналога;
Excalidraw НЕ трогать вслепую — CLAUDE.md); волна 2 — `file_upload` (тип
21), `audio_response` (тип 20), `slider`/`number_line` (компонент
`slider.tsx` уже стоит), `image_choice`, `connect_lines`, `crossword`/
`wordsearch`/`anagram`, `sort_into_pairs` (тип 17), `sequence_timeline`
(тип 18), `map_pin` (тип 19); волна 3 — `chemical_equation` (тип 22),
`formula_input` (тип 15, mathlive уже в стеке), `code_answer` (нет в ТЗ,
песочница — отдельный риск); раздел 4 — степпер, вкладки, флеш-карточки/
галерея (`carousel.tsx` уже стоит), точки на картинке, до/после, лента
времени, таблица Менделеева, вывод формулы по шагам; раздел 5 — банк
вопросов, параметризованные задачи, ветвление, режим тренажёра, таймер,
разноуровневость, импорт вопросов, стартовая библиотека материалов.

**Э13 доп. 5 (2026-09-08) — материал слайдами.** Запрос пользователя:
материал не страница с прокруткой, а слайды; предпросмотр слайдами; методист
видит, на каком слайде; авторазбивка (задания/структура сами делятся,
без «одно задание — один слайд»); весь функционал прежний (пометки учителя
и т.д.); при открытии материала ученика учитель попадает на слайд, где
ученик сейчас.

- **Хранение не менялось** — `materialSchema.blocks` остаётся плоским
  списком. «Слайды» — чистая `paginateMaterial()` / `assignSlides()`
  (`packages/shared/src/materials.ts`, рядом со `stripMaterialAnswerKeys`)
  поверх того же списка: границы = `page_break` (§6.2 ТЗ, блок уже был) +
  заголовок H1/H2 + мягкий потолок вопросов (3) + жёсткий потолок блоков
  (8); группа-конструкция не рвётся. 13 юнит-тестов.
- `materialSettingsSchema.layout: "scroll" | "slides"` (`.default("slides")`
  — новый принцип; старые материалы читаются как `slides`). Тумблер «Как
  проходить» в настройках материала.
- Фронт: `SlideDeck.tsx` (листалка: точки, ←/→, счётчик, `overlay`-слот для
  слоя пометок ВНУТРИ `relative`-бокса слайда — кнопки листания слоем не
  перекрыты). `MaterialPlayer`/`ActivityPlayer` (ученик), `PreviewSheet`
  (редактор), `StudentAttemptView`/`SelectedStudentView` (учитель на уроке)
  — все через `SlideDeck` при `layout==="slides"`, иначе прежняя прокрутка.
- Редактор: `editor/slide-markers.ts` — ProseMirror-декорации «Слайд N»
  прямо в полотне (единицы разбивки из узлов PM → та же `assignSlides`);
  чип «Слайдов: N» в шапке; редактор ремонтируется по `key` при смене
  `layout`.
- Позиция ученика: `PUT /activities/:id/my/position {blockId}` → Redis с
  TTL («кеш, можно потерять» — потеря = открыть с 1-го слайда). `blockId`
  (не индекс) переживает перенарезку. Отдаётся в `MyActivity.currentBlockId`
  (ученик резюмится), `ActivityStudentAttempt.currentBlockId` (учитель
  открывает на слайде ученика, фиксируется 1 раз — потом листает сам),
  `StudentProgress.currentBlockId` (панель класса). Фронт шлёт debounced
  при смене слайда.
- `MaterialAnnotationLayer`: штрих, чей блок-якорь не на текущем слайде, не
  рисуется и не стирается; «Стереть» ограничено текущим слайдом.
- **Проверено руками** (dev-стек поднят: API+Redis+LiveKit+web): редактор —
  метки слайдов, чип, предпросмотр-листалка, клавиши; гость в комнате —
  задание слайдами, навигация; позиция гостя долетает до `/progress` и
  `/students/:id` учителя. Тесты: shared 73, api 393; `pnpm build` чисто
  (shared/api/web); depcheck чисто (331). Не закоммичено.
