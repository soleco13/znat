# Текущий этап: Э6 — Видео учеников

Начато 2026-09-01, продолжено в ТОЙ ЖЕ сессии, что и Э5 — отступление от
правила «один этап = один контекст» (§1.1 ПЛАН.md) по явному решению
пользователя. Перед стартом Э6 сессия сначала закрыла хвост Э5: работа
Э5.1–Э5.5 лежала в рабочей копии незакоммиченной (осталась так с прошлой
сессии, оборванной `/clear`), была перепроверена (`pnpm -r
typecheck`/`test`/`depcheck` — 118/118 бэкенда, всё зелёное) и разбита на 5
отдельных коммитов по образцу Э4 (один коммит на подзадачу). Э5.6 и гейт
Э5 (`livekit-cli load-test`, замер трафика/CPU через Grafana MCP) остаются
заблокированными — нет Docker/livekit-cli/Grafana MCP в этой среде (см.
архив Э5 ниже и памятку project-video-platform-env-gaps). Пользователь
явно попросил не открывать под это отдельную Linux-сессию, а начать Э6
сразу здесь — решение зафиксировано, как и аналогичное для перехода
Э3→Э4 (см. архив Э4).

Как и на Э4/Э5, live-проверка через `docker compose up`/`livekit-cli
load-test` в этой среде невозможна — упор на typecheck/юнит-тесты/чтение
исходников LiveKit SDK, полная проверка (три сценария гейта Э6, официальный
потолок платформы) — на Linux.

**Область «не делегировать вслепую» (CLAUDE.md) покрывает весь этап Э6**:
логика прав/выдачи токенов — `media/service.ts` (гранты, включая новое
право `canPublishVideo`) и `rooms/service.ts` (право выдаёт учитель), плюс
клиентские опции `<LiveKitRoom>` и логика выбора видимой сетки
(`apps/web/src/features/room/RoomPage.tsx`, `TeacherVideoTile.tsx`, будущий
компонент сетки Э6.2/6.3). Каждая такая задача читается и пишется построчно
самим Claude Code в этой сессии, не через делегирование сабагентам.
...
- [ ] Э6.2 Жёсткий лимит: не более 9 видимых видео, подписка только на видимые.
- [ ] Э6.3 Выбор видимых: активные говорящие + закреплённые учителем.
- [ ] Э6.4 Режимы урока (§5.3 ТЗ): Лекция / Обсуждение / Работа над заданием / У доски.
- [ ] Э6.5 Ограничитель на уровне сервера при превышении суммарного трафика.
- [ ] Э6.6 Метрика «трафик по урокам» в Grafana, алерт на 600 Мбит/с.
