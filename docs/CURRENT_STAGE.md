# Текущий этап: Э7 — Демонстрация экрана

Начато 2026-09-02, продолжено в ТОЙ ЖЕ сессии, что и Э5/Э6 — отступление от
правила «один этап = один контекст» (§1.1 ПЛАН.md) по явному решению
пользователя (тот же паттерн, что и для перехода Э5→Э6, см. архив Э6 ниже):
Э6 (Видео учеников) полностью закрыт технически — все 6 задач Э6.1–Э6.6,
132/132 теста бэкенда на момент закрытия, гейт Э6 (`livekit-cli load-test`,
три сценария, официальный потолок платформы) остаётся заблокированным — нет
Docker/livekit-cli/Grafana MCP в этой среде (см. архив Э6 и памятку
project-video-platform-env-gaps). Пользователь явно попросил начать Э7 в
этой же сессии, не открывая отдельную Linux-сессию под гейт.

Как и на Э3–Э6, live-проверка через `docker compose up`/`livekit-cli
load-test` в этой среде невозможна — упор на typecheck/юнит-тесты/чтение
исходников LiveKit SDK, полная проверка (гейт Э7: демонстрация на 30
человек не поднимает трафик выше потолка Э6) — на Linux.

**Область «не делегировать вслепую» (CLAUDE.md) покрывает весь этап Э7**:
логика прав/выдачи токенов — `media/service.ts` (грант `SCREEN_SHARE` по
праву `canShareScreen`) и `rooms/service.ts` (вебхук-логика максимума 1
демонстрации, приоритета учителю, автопереключения режима урока), плюс
клиентская публикация (`ScreenShareControls.tsx`, качество/`contentHint`).
Каждая такая задача читается и пишется построчно самим Claude Code в этой
сессии, не через делегирование сабагентам.
